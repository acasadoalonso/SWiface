...
if os.path.isfile(compfile):
    ...
    login = 'user %s pass %s vers Silent-Wings-Interface %s %s' % (
        config.APRS_USER, config.APRS_PASSCODE, pgmversion, filter)
else:
    login = 'user %s pass %s vers Silent-Wings-Interface %s %s' % (
        config.APRS_USER, config.APRS_PASSCODE, pgmversion, config.APRS_FILTER_DETAILS)
    ...
    while True:
        ...
        if (current_time - keepalive_time) > 180:        # keepalives every 3 mins
            ...
            try:					
                ...
                print((
                    'Something\'s wrong with interface functions Exception type is %s' % (repr(e))))
...
# ------------------------------------------------------- main loop ------------------------------------- #
        ...
        try:
            ...
            if nerr > 20:
                print(
                    "Socket error multiple  Failures.  Orderly closeout, keep alive count:", keepalive_count)
                date = datetime.now()
        ...
        if len(packet_str) == 0:
            ...
            if nerr > 20:
                print(
                    "Read returns zero length string. Failure.  Orderly closeout, keep alive count:", keepalive_count)
                date = datetime.now()
                print("UTC now is: ", date)
                break
            else:
        ...
        if len(packet_str) > 0 and packet_str[0] != "#":
            ...
            if prt:
                ...
                print(roclimb, rot, sensitivity, gps,
                      uniqueid, dist, extpos, source)
            if rot == ' ':
                rot = 0
            if sensitivity == ' ':
                sensitivity = 0
